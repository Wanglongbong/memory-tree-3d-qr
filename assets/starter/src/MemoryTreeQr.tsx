...
type ModuleEntry = {
  scanX: number; scanZ: number; showX: number; showY: number; showZ: number;
  role: QrVisualRole; phase: number; ornamentScale: number;
};
...
export function MemoryTreeQr({ project, onShare, shareLabel }: Props) {
  ...
  useEffect(() => {
    ...
    const entries = createModuleEntries(matrix, project.scene);
    const capMaterial = new THREE.MeshBasicMaterial({ vertexColors: true });
    const capMesh = new THREE.InstancedMesh(new THREE.BoxGeometry(1, 1, 1), capMaterial, entries.length);
    ...
    entries.forEach((entry, index) => { capMesh.setColorAt(index, moduleColors[entry.role]); ornamentMesh.setColorAt(index, moduleColors[entry.role]); });
    ...
    root.add(capMesh, ornamentMesh);

    const scanFloorMaterial = new THREE.MeshBasicMaterial({ color: palette.floor, transparent: true, opacity: 0, depthWrite: false });
    ...
    const organicFloor = new THREE.Mesh(new THREE.CylinderGeometry(matrix.size * (project.scene === "koi" ? 0.4 : 0.34), matrix.size * 0.38, 0.72, project.scene === "koi" ? 48 : 14), organicFloorMaterial);
    organicFloor.position.y = -0.44; root.add(organicFloor);
    ...
    let targetElevation = startsInScan ? Math.PI / 2 : project.scene === "koi" ? 0.68 : 0.56, currentElevation = targetElevation;
    let dragging = false, startX = 0, startY = 0, startYaw = 0, startElevation = targetElevation;
    let scanMix = startsInScan ? 1 : 0, previousMix = -1, previousMode = modeRef.current, last = performance.now();
    const dummy = new THREE.Object3D(); const capScale = new THREE.Vector3(); const ornamentScale = new THREE.Vector3();
    ...
    function updateModuleMorph(mix: number) {
      const eased = smootherstep(mix); const ornamentFade = 1 - smootherstep(THREE.MathUtils.clamp((mix - 0.2) / 0.72, 0, 1));
      entries.forEach((entry, index) => {
        const local = THREE.MathUtils.clamp(eased * 1.08 - entry.phase * 0.08, 0, 1);
        const x = THREE.MathUtils.lerp(entry.showX, entry.scanX, local), y = THREE.MathUtils.lerp(entry.showY, 0.18, local), z = THREE.MathUtils.lerp(entry.showZ, entry.scanZ, local);
        const width = THREE.MathUtils.lerp(project.scene === "koi" ? 0.48 : 0.36, 0.92, local);
        capScale.set(width, THREE.MathUtils.lerp(0.32, 0.38, local), width);
        dummy.position.set(x, y, z); dummy.rotation.set(0, 0, 0); dummy.scale.copy(capScale); dummy.updateMatrix(); capMesh.setMatrixAt(index, dummy.matrix);
        const ornamentSize = entry.ornamentScale * ornamentFade * (1 - local * 0.28);
        ...
        dummy.position.set(x, y + 0.16 * ornamentFade, z); dummy.rotation.set(0, entry.phase * Math.PI * 2, 0); dummy.scale.copy(ornamentScale); dummy.updateMatrix(); ornamentMesh.setMatrixAt(index, dummy.matrix);
      });
      capMesh.instanceMatrix.needsUpdate = true; ornamentMesh.instanceMatrix.needsUpdate = true;
      scanFloorMaterial.opacity = smootherstep(THREE.MathUtils.clamp((mix - 0.38) / 0.62, 0, 1)); scanFloor.visible = scanFloorMaterial.opacity > 0.01;
      organicFloorMaterial.opacity = 1 - smootherstep(THREE.MathUtils.clamp((mix - 0.38) / 0.5, 0, 1)); organicFloor.visible = organicFloorMaterial.opacity > 0.01;
    ...
    function animate(now: number) {
      ...
      if (activeMode !== previousMode) {
        ...
        else { targetElevation = project.scene === "koi" ? 0.68 : 0.56; targetYaw = -0.48; }
      ...
      if (Math.abs(scanMix - previousMix) > 0.001) { updateModuleMorph(scanMix); previousMix = scanMix; }
      ...
      camera.zoom = THREE.MathUtils.lerp(project.scene === "tree" ? 1.36 : project.scene === "lantern" ? 1.2 : 1.08, 1, smootherstep(scanMix)); camera.updateProjectionMatrix(); camera.lookAt(0, lookHeight, 0);
      if (!reducedMotion) { particles.rotation.y += delta * 0.07; animateSupportLayer(supportLayer, now, project.scene); }
      renderer.render(scene, camera); frame = requestAnimationFrame(animate);
    }
    setStatus("Kéo ngang để xoay · kéo dọc để nhìn mã hình thành"); frame = requestAnimationFrame(animate);
  ...
  return <article className="memory-card" data-time={project.time} data-scene={project.scene}>
    ...
    <div className="memory-stage" data-mode={mode} style={{ "--scan-floor": palette.floor } as CSSProperties}><div ref={mountRef} className="memory-render"/><div className="world-label"><small>{project.season.toUpperCase()} · {project.time === "night" ? "NIGHT" : "DAY"}</small><strong>{sceneNames[project.scene]}</strong><span>{mode === "scan" ? "QR đa sắc · giữ màn hình thẳng và đủ sáng" : status}</span></div><div className="scan-corners" aria-hidden="true"><i/><i/><i/><i/></div></div>
    ...
    <p className="security-line">⌾ Sàn và module cùng bảng màu; mọi màu QR được tự cân lại để giữ tương phản quét.</p>
  </article>;
}

function createModuleEntries(matrix: QrMatrix, scene: SceneKind): ModuleEntry[] {
  const entries: ModuleEntry[] = []; const centre = (matrix.size - 1) / 2;
  matrix.modules.forEach((row, r) => row.forEach((dark, c) => {
    if (!dark) return;
    const scanX = c - centre, scanZ = r - centre, role = classifyDarkModule(r, c, matrix.size), seed = hash2(r, c), phase = seed;
    const compressed = scene === "tree" ? 0.31 : scene === "lantern" ? 0.44 : 0.62;
    const showX = scanX * compressed, showZ = scanZ * compressed;
    const radius = Math.hypot(scanX, scanZ) / (matrix.size * 0.72);
    const dome = Math.sqrt(Math.max(0, 1 - Math.min(1, radius) ** 2));
    const showY = scene === "tree" ? matrix.size * (0.18 + dome * 0.19) + (seed - 0.5) * 1.4 : scene === "lantern" ? matrix.size * (0.09 + dome * 0.12) + (seed - 0.5) * 1.1 : 0.58 + seed * 0.45;
    entries.push({ scanX, scanZ, showX, showY, showZ, role, phase, ornamentScale: scene === "tree" ? 0.72 + seed * 0.34 : scene === "lantern" ? 0.62 + seed * 0.24 : 0.66 + seed * 0.3 });
  }));
  return entries;
}
...
function buildSupportLayer(scene: SceneKind, entries: ModuleEntry[], palette: ScenePalette, size: number) {
  ...
  if (scene === "tree") {
    const height = size * 0.22; const trunk = new THREE.Mesh(new THREE.CylinderGeometry(size * 0.022, size * 0.045, height, 8), main); trunk.position.y = height / 2; group.add(trunk);
    const stride = Math.max(1, Math.floor(entries.length / 24));
    entries.filter((_, index) => index % stride === 0).slice(0, 24).forEach((entry) => addCylinderBetween(group, main, new THREE.Vector3(0, height * 0.64, 0), new THREE.Vector3(entry.showX, entry.showY - 0.5, entry.showZ), size * 0.008));
